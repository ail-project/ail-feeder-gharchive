...
parser.add_argument("-l", "--list-leak", help="list of word to search. If no list is give, all commit message will be add", action="store_true")
...
if args.list-leak:
    with open(args.list-leak, "r") as read_file:
...
for element in data:
    if element["type"] == "PushEvent":
        ...
        if args.org or args.users:
            if "org" in element and arg.org == element["org"]["login"]:
                ...
                if args.list-leak:
                    ...
                    for i in range(0, len(element["payload"]["commits"])):
                        for lines in list_leak:
                            if lines in element["payload"]["commits"][i]["message"]:
                                ele_list.append(element)
                                flagList = True
                                break
                        if flagList:
    ...
    for i in range(0,len(element["payload"]["commits"])):
        ...
        if "message" in json_api:
            ...
            for j in range(0,len(json_api["files"])):
                ...
                with open("B:\\Téléchargement\\Travail\\GhArchive\\patch.json", "a") as write_file:
        ...
        with open("B:\\Téléchargement\\Travail\\GhArchive\\commit.json", "a") as write_file:
